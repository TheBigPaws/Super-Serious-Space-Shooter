...
bool ModelsLoader::load(char* modelFilename, char* textureFilename)
{
	bool result;
...
bool ModelsLoader::loadModel(char* filename)
{
...
#pragma region unroll_data
	...
	for (int i = 1; i < faces.size(); i += 3) {
		...
		texCoords.push_back(texCs.at(faces.at(i) - 1).x);
		texCoords.push_back(texCs.at(faces.at(i) - 1).y);
	...
	for (int i = 2; i < faces.size(); i += 3) {
		...
		normals.push_back(norms.at(faces.at(i) - 1).x);
		normals.push_back(norms.at(faces.at(i) - 1).y);
		normals.push_back(norms.at(faces.at(i) - 1).z);
...
void ModelsLoader::loadTexture(char* filename)
{
