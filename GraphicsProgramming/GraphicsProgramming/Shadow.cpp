...
Shadow::Shadow() {
	...
	generateShadowMatrix(shadowMatrix, OutsideLight_Position, planeverts);
}
...
void Shadow::generateShadowMatrix(float* shadowMatrix, float light_pos[4], GLfloat floor[12]) {
	...
	//below function only needs to be passed once as the shadow is static
}
...
void Shadow::generateOutsideShadows(bool displayAll) {
	...
	Structures.generateStructures(0);
	FurnitureAndDetails.renderOutside(displayAll);
	glPopMatrix();
